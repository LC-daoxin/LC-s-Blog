...
{
	ajaxFinish = true;
	jQuery("#ajax_loading").fadeOut("slow");
}

function registerTableCheckbox(table,className)
{
	jQuery("."+className,jQuery("#"+table)).click
	(
		function()
		{
			jQuery(this.parentNode.parentNode).toggleClass("select");
		}
	);

	jQuery("tr",jQuery("#"+table)).mouseover
	(
		function()
		{
			if(jQuery("."+className,jQuery(this)).attr("checked") != true && !jQuery(this).hasClass("heading"))
			{
	...
	jQuery("tr",jQuery("#"+table)).mouseout
	(
		function()
		{
			if(jQuery("."+className,jQuery(this)).attr("checked") != true && !jQuery(this).hasClass("heading"))
...
{
	jQuery("."+className,jQuery("#"+table)).each(
		function()
		{
			jQuery(this).attr("checked",true);
			jQuery(this.parentNode.parentNode).addClass("select");
		}
	);
}
...
function selectTableNone(table,className)
{
	jQuery("."+className,jQuery("#"+table)).each(
		function()
		{
			...
			jQuery(this.parentNode.parentNode).removeClass("select");
		}
	);
}
	...
	jQuery("."+className,jQuery("#"+table)).each(
		function()
		{
			if(jQuery(this).attr("checked") == true)
			{
				jQuery(this).attr("checked",false);
                jQuery(this.parentNode.parentNode).removeClass("select");
                jQuery(this.parentNode.parentNode).removeClass("hover");
			}
			else
			{
				jQuery(this).attr("checked",true);
...
function registerInputFocus(element)
{
	...
	jQuery("textarea.text",element).focus
	(
		function()
		{
	...
	jQuery("span.button",element).mouseover
	(
		function()
		{
			jQuery(this).addClass("focus");
	...
	jQuery("span.button",element).mouseout
	(
		function()
		{
			jQuery(this).removeClass("focus");
	...
	jQuery("span.button",element).mousedown
	(
		function()
		{
			jQuery(this).removeClass("click");
	...
	jQuery("span.button",element).mouseup
	(
		function()
		{
			jQuery(this).removeClass("click");
...
function magikeConfirm(el)
{
	...
	p.text(jQuery(el).attr("msg"));
	div.append(p);
	
	magikeUI.createPopup({title: mgTruncate(jQuery(el).attr("msg"),20,'...'),center: true,block:true,
...
{
	magikeLocation(jQuery(confirmElement).attr('rel'));
}
...
function magikeSubmitConfirm(msg,el,action)
{
	...
	if(action)
	{
		jQuery('input[@name=do]').val(action);
	}
...
{
	if(jQuery('input:checked',jQuery('#' + submitConfirmElement)).val())
	{
		document.getElementById(submitConfirmElement).submit();
	}
	else
	{
		jQuery('.magikeShadow').remove();
		jQuery('.magikePopup').remove();
	}
}
...
MagikeUI.prototype = {
	...
	createPopup: function(args)
	{
		...
		//创建窗口外框
		var popupWindow = jQuery(document.createElement("div"));
		...
		//创建窗口标题
		var popupTitle = jQuery(document.createElement("div"));
		...
		//创建窗口关闭按钮
		var closeBar = jQuery(document.createElement("span"));
		...
		jQuery(document.body).append(popupWindow);
		...
		popupWindow.mousedown(
			function()
			{
				jQuery('.magikePopup').css('z-index',995);
				jQuery('.magikePopupShadow').css('z-index',996);
				jQuery('.magikePopupContent').css('z-index',997);
		...
		jQuery('.magikePopupClose',popupWindow).click(
			function()
			{
				popupWindow.remove();
				if(popupWindow.shadow)
	...
	createShadow: function()
	{
		...
		jQuery(document.body).append(shadow);
...
function fixCssHack()
{
	...
	jQuery(".message").dblclick(function(){jQuery(this).hide();});
	jQuery(".proc").click(function(){jQuery(this).hide();});
	
	jQuery(window).unload(
		function()
		{
			jQuery('span.button').unbind();
...
function magikeValidator(url,mod)
{
	...
	s = jQuery('.validate-me').serialize();
	jQuery.ajax({
		type: 'POST',
		url: url + '?mod=' + mod,
		data: s,
